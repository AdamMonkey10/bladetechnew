-- Update the timesheet tracking function to use simplified 3-tier escalation
...
BEGIN
    ...
    SELECT DISTINCT 
        ...
        CASE 
            WHEN sr.id IS NULL THEN (CURRENT_DATE - DATE(cte.clock_in))::INTEGER
            ELSE 0
        END as days_overdue
    ...
    LOOP
        ...
        IF NOT tracking_record.timesheet_submitted THEN
            ...
            ELSE
                -- Update days overdue and escalation level using simplified 3-tier system
                UPDATE public.timesheet_tracking 
                SET 
                    days_overdue = (CURRENT_DATE - work_date)::INTEGER,
                    escalation_level = CASE 
                        WHEN (CURRENT_DATE - work_date)::INTEGER = 0 THEN 'none'
                        WHEN (CURRENT_DATE - work_date)::INTEGER = 1 THEN 'late'
                        WHEN (CURRENT_DATE - work_date)::INTEGER >= 2 THEN 'critical'
                        ELSE 'none'
                    END,
                    updated_at = now()
                WHERE id = tracking_record.id;
