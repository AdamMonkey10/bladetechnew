-- Fix the fix_missing_hours function to remove reference to non-existent updated_at column
CREATE OR REPLACE FUNCTION public.fix_missing_hours()
 RETURNS TABLE(record_id uuid, activity_type text, units_produced integer, original_hours numeric, estimated_hours numeric, corrected boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
