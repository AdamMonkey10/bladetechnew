-- Fix the update_timesheet_tracking function with correct date arithmetic
            ...
            ELSE
                ...
                SET 
                    ...
                    escalation_level = CASE 
                        WHEN (CURRENT_DATE - work_date)::INTEGER >= 6 THEN 'critical'
                        WHEN (CURRENT_DATE - work_date)::INTEGER >= 3 THEN 'urgent'
                        WHEN (CURRENT_DATE - work_date)::INTEGER >= 1 THEN 'warning'
                        ELSE 'none'
                    END,
                    updated_at = now()
                WHERE id = tracking_record.id;
