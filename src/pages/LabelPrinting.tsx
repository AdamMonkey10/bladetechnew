...
import { useLabelSessions } from '@/hooks/useLabelSessions';
import { LaserForm } from '@/components/label-printing/LaserForm';
...
export default function LabelPrinting() {
  ...
  const { sessions, loading: sessionsLoading, saveSession, getSession } = useLabelSessions();
  const { products, operators, machines, isLoading: referenceLoading, invalidateReferenceData } = useReferenceData();
  const { printerSettings, loading: printerLoading } = usePrinterSettings();
  ...
  const sendZPLToPrinter = async (zpl: string) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Print Success",
          description: "Label sent to print server successfully",
        });
      } else {
        const errorText = await response.text();
        throw new Error(`Print server failed with status: ${response.status} - ${errorText}`);
      }
    } catch (error) {
      toast({
        title: "Print Server Error",
        description: `Failed to send to ${printUrl}: ${error}`,
        variant: "destructive",
      });
    }
  };

  // Handle session updates from laser forms
  const handleSessionUpdate = (laserMachineId: string, sessionData: any) => {
    saveSession(laserMachineId, sessionData);
  };

  const isLoading = referenceLoading || sessionsLoading || printerLoading;
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading laser systems...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Label Printing - Laser Stations</h1>
          <p className="text-muted-foreground">Manage label printing across three laser cutting stations</p>
        </div>
        <Button 
          variant="outline" 
      ...
      {/* Three Laser Forms Side by Side */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        {laserMachines.map((machine) => (
          <LaserForm
            ...
            products={products}
            operators={operators}
            sessionData={getSession(machine.id)}
            onSessionUpdate={(data) => handleSessionUpdate(machine.id, data)}
            onPrint={sendZPLToPrinter}
            companyLogoZPL={COMPANY_LOGO_ZPL}
          />
        ))}
      </div>
    </div>
